...
describe('version metadata', () => {
  it('keeps the upstream package version and shows fork metadata separately', () => {
    ...
    expect(getPluginVersionDisplay()).toBe('upstream 2026.4.1 / fork turingclaw 2026.5.14.2');
  });

  it('includes fork metadata in the User-Agent without spaces', () => {
    expect(getUserAgent()).toMatch(/^openclaw-lark\/2026\.4\.1\+turingclaw-2026\.5\.14\.2\/(?:mac|linux|windows)$/u);
  });
});
