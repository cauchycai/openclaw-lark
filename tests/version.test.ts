...
describe('version metadata', () => {
  it('keeps the upstream package version and shows fork metadata separately', () => {
    ...
    expect(getPluginVersionDisplay()).toBe('upstream 2026.6.3 / fork turingclaw 2026.6.3.1');
  });

  it('includes fork metadata in the User-Agent without spaces', () => {
    expect(getUserAgent()).toMatch(/^openclaw-lark\/2026\.6\.3\+turingclaw-2026\.6\.3\.1\/(?:mac|linux|windows)$/u);
  });
});
