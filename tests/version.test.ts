...
describe('version metadata', () => {
  it('keeps the upstream package version and shows fork metadata separately', () => {
    ...
    expect(getPluginVersionDisplay()).toBe('upstream 2026.6.3 / fork turingclaw 2026.6.3.2');
  });

  it('includes fork metadata in the User-Agent without spaces', () => {
    expect(getUserAgent()).toMatch(/^openclaw-lark\/2026\.6\.3\+turingclaw-2026\.6\.3\.2\/(?:mac|linux|windows)$/u);
  });
});
