...
describe('version metadata', () => {
  it('keeps the upstream package version and shows fork metadata separately', () => {
    ...
    expect(getPluginVersionDisplay()).toBe('upstream 2026.6.2 / fork turingclaw 2026.6.2.4');
  });

  it('includes fork metadata in the User-Agent without spaces', () => {
    expect(getUserAgent()).toMatch(/^openclaw-lark\/2026\.6\.2\+turingclaw-2026\.6\.2\.4\/(?:mac|linux|windows)$/u);
  });
});
